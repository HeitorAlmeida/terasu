Shader "Custom/SpriteFakeLit"
{
	...
	Properties
	{
        ...
        _LightOffset ("Light Offset", Vector) = (0.0, 0.0, 0.0, 0.0)
        _LightRadius ("Light Radius", Float) = 0.0
        _LightIntensity ("Light Intensity", Float) = 1.0
        _NormalTex ("Sprite Normals", 2D) = "white" {}
        _ManualFlip ("Manual Flip", Vector) = (1.0, 1.0, 1.0, 1.0)
	}
	SubShader
	{
		Tags
		...
		Pass
		{
			...
			uniform float4 _LightOffset;
			uniform float _LightRadius;
			uniform float _LightIntensity;
			uniform sampler2D _NormalTex;
			uniform float4 _ManualFlip;
			...
			fixed4 frag(v2f IN) : SV_Target
			{
				...
				fixed4 normalColor = tex2D(_NormalTex, IN.texcoord);
				//halmeida - if no normal map texture was assigned, the lookup will return white. White will never be retrieved
				//from a valid normal map. We have to keep the shader working, ignoring the effects of the normal map. Further below,
				//a treatment for the absence of normal map is implemented using the minimum component of the color.
				float minComponent = min(normalColor.r, min(normalColor.g, normalColor.b));
				float3 normalVertex = (normalColor * 2.0) - 1.0;
				float3 normalDirection = normalize(normalVertex);
				//halmeida - if the sprite is flipped, we have to reverse the normal direction.
				normalDirection.xy *= _ManualFlip.xy;
				float3 vertexToLight = _LightOffset.xyz - float3(IN.objectSpaceVertex.xy, 0.0);
				float3 lightDirection = normalize(vertexToLight);
				float diffuseAngleFactor = max(0.0, dot(normalDirection, lightDirection));
				//halmeida - if the minimum component of the color was 1.0, the color received was white and there was no normal map.
				//In this case, the diffuseAngleFactor becomes 1.0, never reducing the diffuse lighting. If the color is not white,
				//then the floor of the minimum component is 0.0 and the diffuse angle factor of lighting reduction is kept.
				diffuseAngleFactor = max(diffuseAngleFactor, floor(minComponent));
				float distanceToLight = length(vertexToLight);
				float intensity = max((_LightRadius - distanceToLight), 0.0);
				intensity *= max(_LightIntensity, 1.0);
				c.rgb += diffuseAngleFactor * _LightColor.rgb * intensity;
				c.rgb *= c.a;
