	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag(v2f IN) : SV_Target
			{
				...
				fixed originalContentAlpha = c.a;
				c *= IN.color;
				c.rgb += _ColorToAdd.xyz;

				float2 uv = IN.texcoord;
				fixed4 pushedUp = tex2D(_MainTex, float2( uv.x, uv.y-_OutlineWidth ));
				fixed4 pushedLeft = tex2D(_MainTex, float2( uv.x+_OutlineWidth, uv.y ));
				fixed4 pushedDown = tex2D(_MainTex, float2( uv.x, uv.y+_OutlineWidth ));
				fixed4 pushedRight = tex2D(_MainTex, float2( uv.x-_OutlineWidth, uv.y ));
				fixed outlineAlpha = min(1.0, pushedUp.a + pushedLeft.a + pushedDown.a + pushedRight.a);

				//halmeida - if the outline wraps around something, we have to remove this something from the area
				//covered by the outline. We will also wrap around semi-transparent pixels, as if they were solid.
				//This will allow us to outline ghostly figures, if we ever need to.
				fixed noOutline = ceil(originalContentAlpha);
				outlineAlpha -= noOutline;

				c.rgb = (noOutline * c.rgb) + ((1.0 - noOutline) * _OutlineColor.rgb);
				c.a = (noOutline * (originalContentAlpha * IN.color.a)) + ((1.0 - noOutline) * (outlineAlpha * _OutlineColor.a));
